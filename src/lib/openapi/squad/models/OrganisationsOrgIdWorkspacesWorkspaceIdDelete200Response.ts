/* tslint:disable */
/* eslint-disable */
/**
 * Squad API
 * API for managing Squad resources
 *
 * The version of the OpenAPI document: 0.14.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import type { OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseData } from "./OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseData.js";
import {
  OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseDataFromJSON,
  OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseDataToJSON,
} from "./OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseData.js";

/**
 * Response containing a success message
 * @export
 * @interface OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response
 */
export interface OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response {
  /**
   *
   * @type {OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseData}
   * @memberof OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response
   */
  data: OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseData;
}

/**
 * Check if a given object implements the OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response interface.
 */
export function instanceOfOrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response(
  value: object,
): value is OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response {
  if (!("data" in value) || value["data"] === undefined) return false;
  return true;
}

export function OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseFromJSON(
  json: any,
): OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response {
  return OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseFromJSONTyped(
    json,
    false,
  );
}

export function OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean,
): OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response {
  if (json == null) {
    return json;
  }
  return {
    data: OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseDataFromJSON(
      json["data"],
    ),
  };
}

export function OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseToJSON(
  json: any,
): OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response {
  return OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseToJSONTyped(
    json,
    false,
  );
}

export function OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseToJSONTyped(
  value?: OrganisationsOrgIdWorkspacesWorkspaceIdDelete200Response | null,
  ignoreDiscriminator: boolean = false,
): any {
  if (value == null) {
    return value;
  }

  return {
    data: OrganisationsOrgIdWorkspacesWorkspaceIdDelete200ResponseDataToJSON(
      value["data"],
    ),
  };
}
