  ...
  server.tool(
    ...
    async (params, ctx) => {
      try {
        ...
        return toolSuccess({
          ...outcome,
          opportunities: outcome.opportunities?.map((o: { id: string; title: string; status: string }) => ({
            id: o.id,
            title: o.title,
            status: o.status,
          })),
          solutions: outcome.solutions?.map((s: { id: string; title: string; status: string }) => ({
            id: s.id,
            title: s.title,
            status: s.status,
          })),
          insights: outcome.insights?.map((i: { id: string; title: string; type: string }) => ({
            id: i.id,
            title: i.title,
            type: i.type,
          })),
