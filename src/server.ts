...
if (
  !process.env.PROPELAUTH_CLIENT_ID ||
  !process.env.PROPELAUTH_CLIENT_SECRET
) {
  logger.fatal(
...
server.app.get("/.well-known/openid-configuration", async c => {
  try {
    const response = await fetch(
      `${AUTH_URL}/oauth/2.1/.well-known/openid-configuration`,
    );
    if (!response.ok) {
      logger.error(
        { status: response.status },
        "OpenID discovery proxy failed",
      );
